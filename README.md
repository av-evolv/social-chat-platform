...
Expo native peer versions are constrained to its SDK 57 compatibility matrix to avoid duplicate native modules; the `xcode` UUID override retains its CommonJS API while selecting the patched release. One upstream Router query-decoder advisory remains tracked in [#30](https://github.com/av-evolv/social-chat-platform/issues/30), blocking external deep-link/auth flows and public distribution. Do not apply incompatible major overrides or `npm audit fix --force`.
