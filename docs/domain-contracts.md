...
## Planned circle admission extensions

See [friends and circle admission](../social_chat_platform_plan.md#351-friends-and-circle-admission-planned) for #51/#23 and [calendar consent](../social_chat_platform_plan.md#352-calendar-acceptance-of-an-associated-circle-planned) for #41. A current authorized administrator may add an accepted friend or verified active co-organisation member without separate circle confirmation. Evaluate canonical identity, current relationship, inviter authority, target revision and prior leave/removal atomically. This grants MEMBER circle membership only; explicit conversation exclusions and cryptographic admission remain independent.

A verified intended recipient may receive a revocable preview capability for circle member display identities, description and optional logo/header before accepting. This does not grant audience-source enumeration or general circle read access. Event association never continuously derives circle membership: the external-calendar exception consumes specific disclosed circle consent attached to a validated invitation, not an EVENT audience edge. Store that consent separately from authenticated access until verified identity eligibility/claim, with audited idempotent reconciliation.
