...
The intended coordination surface is **Larynx delivery**, an organisation-level GitHub Project linked to this repository, alongside milestones. Its URL and verified setup/access status are tracked in [#31](https://github.com/av-evolv/social-chat-platform/issues/31); follow the fallback below until setup is complete. Milestones describe delivery goals; issues are the work items; pull requests hold detailed plans, implementation and verification; the project provides shared status, ownership and timeline views. Do not create parallel draft tasks for existing issues.
